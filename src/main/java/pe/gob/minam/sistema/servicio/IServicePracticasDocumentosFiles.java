/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package pe.gob.minam.sistema.servicio;

import java.util.List;
import pe.gob.minam.common.service.excepcion.ServiceException;
import pe.gob.minam.seguridad.common.navegacion.ResultadoServicio;
import pe.gob.minam.sistema.entidades.PracticasDocumentosFiles;

/**
 *
 * @author jmarinc
 */
public interface IServicePracticasDocumentosFiles {
    
     public ResultadoServicio guardarPracticasDocumentosFiles(PracticasDocumentosFiles PracticasDocumentosFiles);
    
    public ResultadoServicio actualizarPracticasDocumentosFiles(PracticasDocumentosFiles PracticasDocumentosFiles);
    
    public ResultadoServicio eliminarPracticasDocumentosFiles(PracticasDocumentosFiles PracticasDocumentosFiles);

    public List<PracticasDocumentosFiles> listarPracticasDocumentosFiles() throws ServiceException;
    
    public PracticasDocumentosFiles getPracticasDocumentosFiles(Integer id) throws ServiceException;
}
